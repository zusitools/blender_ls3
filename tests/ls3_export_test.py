...
class TestLs3Export(unittest.TestCase):
  ...
  def test_negative_scale(self):
    ...
    self.assertEqual(8, len(p_nodes))
    self.assertEqual(8, len(n_nodes))
  ...
  def test_too_many_vertices(self):
    ...
    # Different #subsets compared to Blender 2.8 because the tessfaces API operates on tris and quads
    # (-> 4*65536 quads -> 262144 verts) while 2.8 operates on tris only (-> 3*131072 tris -> 393216 verts).
    self.assertEqual(4, len(subset_nodes))
  ...
  def test_coordinate_system(self):
    ...
    for coordinate_system in ("0", "1"):
      with self.subTest(coordinate_system=coordinate_system):
        ...
        for i in range(3):
          for j in range(6):
            self.assertAlmostEqual(expected_coords[i][j], coords[i][j], places=1, msg="i={} j={}".format(i, j))
        ...
        for i in range(3):
          self.assertAlmostEqual(expected_coords[i], coords[i], places=1, msg="i={}".format(i, ))
